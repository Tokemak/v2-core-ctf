...
interface IrswETH is IERC20 {
    ...
    /**
     * @dev Error thrown when repricing the rate and distributing rewards to NOs when they are no active validators.
     * This condition should never happen; it means that no active validators were running but we still have rewards,
     * despite this it's still here for security
     */
    error NoActiveValidators();

    /**
     * @dev Error thrown when updating the reward percentage for either the NOs or the swell treasury and the update
     * will cause the NO percentage + swell treasury percentage to exceed 100%.
     */
    error RewardPercentageTotalOverflow();

    /**
     * @dev Thrown when calling the reprice function and not enough time has elapsed between the previous repriace and
     * the current reprice.
    ...
    /**
     * @dev Thrown during repricing when the difference in rswETH supplied to repricing compared to the actual supply is
     * too great
     ...
     */
    error RepricerswETHDifferenceTooLarge(uint256 repricerswETHDiff, uint256 maximumrswETHRepriceDiff);
     ...
     */
    event ETHWithdrawn(address indexed to, uint256 rswETHBurned, uint256 ethReturned);
    ...
    event MaximumRepricerswETHDifferencePercentageUpdated(
        uint256 _oldMaximumRepricerswETHDifferencePercentage, uint256 _newMaximumRepricerswETHDifferencePercentage
    );
    ...
    event MaximumRepriceDifferencePercentageUpdated(
        uint256 _oldMaximumRepriceDifferencePercentage, uint256 _newMaximumRepriceDifferencePercentage
    );
    ...
    /**
     * @dev Returns the current RswETH to ETH rate, returns 1:1 if no reprice has occurred otherwise it returns the
     * rswETHToETHRateFixed rate.
    ...
    /**
     * @dev Sets the maximum percentage allowable difference in rswETH supplied to repricing compared to current rswETH
     * supply.
     * @notice Only a platform admin can call this function.
    ...
    /**
     * //  * TODO: Reword
     * @dev This method reprices the rswETH -> ETH rate, this will be called via an offchain service on a regular
     * interval, likely ~1 day. The rswETH total supply is passed as an argument to avoid a potential race conditions
     * between the off-chain reserve calculations and the on-chain repricing
     * @dev This method also mints a percentage of rswETH as rewards to be claimed by NO's and the swell treasury. The
     * formula for determining the amount of rswETH to mint is the following: swETHToMint = (swETHSupply * newETHRewards
     * * feeRate) / (preRewardETHReserves - newETHRewards * feeRate + newETHRewards)
     * @dev The formula is quite complicated because it needs to factor in the updated exchange rate whilst it
     * calculates the amount of rswETH rewards to mint. This ensures the rewards aren't double-minted and are backed by
     * ETH.
     ...
     */
    function reprice(uint256 _preRewardETHReserves, uint256 _newETHRewards, uint256 _rswETHTotalSupply) external;
}
