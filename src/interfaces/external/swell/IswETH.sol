...
interface IswETH is IERC20 {
     ...
     * This condition should never happen; it means that no active validators were running but we still
     * have rewards, despite this it's still here for security
     */
    ...
    /**
     * @dev Thrown during repricing when the difference in swETH supplied to repricing compared to the actual supply is
     * too great
     ...
     */
    event ETHDepositReceived(address indexed from, uint256 amount, uint256 swETHMinted, uint256 newTotalETHDeposited);
    ...
    event MaximumRepriceswETHDifferencePercentageUpdated(
        uint256 _oldMaximumRepriceswETHDifferencePercentage, uint256 _newMaximumRepriceswETHDifferencePercentage
    );
    ...
    /**
     * @dev Returns the current SwETH to ETH rate, returns 1:1 if no reprice has occurred otherwise it returns the
     * swETHToETHRateFixed rate.
     ...
     * @dev This method also mints a percentage of swETH as rewards to be claimed by NO's and the swell treasury. The
     * formula for determining the amount of swETH to mint is the following: swETHToMint = (swETHSupply * newETHRewards
     ...
     * calculates the amount of swETH rewards to mint. This ensures the rewards aren't double-minted and are backed by
     * ETH.
