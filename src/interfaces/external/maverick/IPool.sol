...
interface IPool {
     ...
     * @param isDelta bool that indicates whether the bin position is
     *        relative to the current bin or an absolute position
     ...
     * @param status pool status.  e.g. locked or unlocked; status values defined in Pool.sol
     * @param binCounter index of the last bin created
     * @param protocolFeeRatio ratio of the swap fee that is kept for the protocol
     */
    struct State {
        int32 activeTick;
        uint8 status;
     ...
     * @param params array of AddLiquidityParams that specify the mode and position of the liquidity
     * @param data callback function that addLiquidity will call so that the caller can transfer tokens
     */
    function addLiquidity(
        uint256 tokenId,
        AddLiquidityParams[] calldata params,
        bytes calldata data
    ) external payable returns (uint256 tokenAAmount, uint256 tokenBAmount, BinDelta[] memory binDeltas);

    /**
     * @notice Transfer liquidity in an array of bins from one nft tokenId to another
     ...
     * @param params array of RemoveLiquidityParams that specify the bins, and amounts
     */
    function removeLiquidity(
        address recipient,
        uint256 tokenId,
        RemoveLiquidityParams[] calldata params
    ) external returns (uint256 tokenAOut, uint256 tokenBOut, BinDelta[] memory binDeltas);
     ...
     * @param sqrtPriceLimit limiting sqrt price of the swap.
     *        A value of 0 indicates no limit. Limit is only engaged for exactOutput=false.
     *        If the limit is reached only part of the input amount will be swapped and the
     *        callback will only require that amount of the swap to be paid.
     * @param data callback function that swap will call so that the caller can transfer tokens
     */
    function swap(
        address recipient,
        uint256 amount,
