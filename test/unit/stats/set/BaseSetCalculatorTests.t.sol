...
contract TestBaseSetCalculator is BaseSetCalculator {
    constructor(
        ISystemRegistry _systemRegistry
    ) BaseSetCalculator(_systemRegistry) { }
}

abstract contract BaseSetCalculatorTests is Test, SystemRegistryMocks, AccessControllerMocks {
    ISystemRegistry internal systemRegistry;
    IAccessController internal accessController;

    BaseSetCalculator internal calculator;
    ...
    function setUp() external {
        ...
        calculator = TestBaseSetCalculator(Clones.clone(address(new TestBaseSetCalculator(systemRegistry))));
    }
...
contract Initialize is BaseSetCalculatorTests {
    ...
    function test_RevertIf_TemplateContract() external {
        BaseSetCalculator.InitData memory initData = getDefaultInitData();
        _mockSystemComponent(systemRegistry, initData.cacheStore);
        BaseSetCalculator localCalc = new TestBaseSetCalculator(systemRegistry);
...
contract ShouldSnapshot is BaseSetCalculatorTests {
    function test_ReturnsFalse() external {
        assertEq(calculator.shouldSnapshot(), false, "val");
    }
}

contract Snapshot is BaseSetCalculatorTests {
