...
contract EtherFiDestinationVaultExtensionTest is Test {
    ...
    // won't be used often
    IDestinationVault constant dv = IDestinationVault(0x148Ca723BefeA7b021C399413b8b7426A4701500);
    ISystemRegistry constant systemRegistry = ISystemRegistry(SYSTEM_REGISTRY_MAINNET);
    address constant etherFiClaim = ETHERFI_LRTSQUARED_CLAIM;
    address constant zeroExSwapper = ZERO_EX_SWAPPER_MAINNET;
    IERC20 constant lrtSquaredToken = IERC20(LRTSQUARED_MAINNET);
    IERC20 constant wethERC20 = IERC20(WETH_MAINNET);
    ...
    function setUp() public virtual {
        ...
        extension = new EtherFiDestinationVaultExtension(systemRegistry, zeroExSwapper, etherFiClaim, lrtSquaredToken);
    ...
    // Returns struct so data can be changed in test
    function _getExtensionData()
        internal
        view
        returns (EtherFiDestinationVaultExtension.EtherFiDVExtensionParams memory dataStruct)
    {
        ...
        dataStruct = EtherFiDestinationVaultExtension.EtherFiDVExtensionParams({
            expectedClaimAmount: claimAmount,
...
contract EtherFiDVExtensionConstructorTest is EtherFiDestinationVaultExtensionTest {
    function test_RevertsWhenZeroAddresses() public {
        ...
        new EtherFiDestinationVaultExtension(systemRegistry, address(0), etherFiClaim, lrtSquaredToken);

        vm.expectRevert(abi.encodeWithSelector(Errors.ZeroAddress.selector, "_claimAddress"));
        new EtherFiDestinationVaultExtension(systemRegistry, zeroExSwapper, address(0), lrtSquaredToken);

        vm.expectRevert(abi.encodeWithSelector(Errors.ZeroAddress.selector, "_claimToken"));
        new EtherFiDestinationVaultExtension(systemRegistry, zeroExSwapper, etherFiClaim, IERC20(address(0)));
    }

    function test_StateSet() public {
        assertEq(extension.asyncSwapper(), zeroExSwapper);
        assertEq(extension.claimAddress(), etherFiClaim);
...
contract EtherFiDVExtensionExecuteTest is EtherFiDestinationVaultExtensionTest {
    function test_RevertIf_NonDVCaller() public {
        bytes memory data = abi.encode(_getExtensionData());
    ...
    function test_RevertIf_claimAmount_Zero() public {
        EtherFiDestinationVaultExtension.EtherFiDVExtensionParams memory params = _getExtensionData();
        params.expectedClaimAmount = 0;
    ...
    function test_RevertIf_ClaimAmount_And_ExpectedAmount_AreNotEqual() public {
        EtherFiDestinationVaultExtension.EtherFiDVExtensionParams memory params = _getExtensionData();
        params.expectedClaimAmount = 10;

        bytes memory data = abi.encode(params);

        vm.expectRevert(
            abi.encodeWithSelector(EtherFiDestinationVaultExtension.InvalidAmountReceived.selector, claimAmount, 10)
        );
        dv.executeExtension(data);
    }
    ...
    function test_RunsProperly() public {
        ...
        emit EtherFiExtensionExecuted(claimAmount, wethAmountReceived);
        dv.executeExtension(data);
