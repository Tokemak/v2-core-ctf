...
// Copied from https://github.com/a16z/erc4626-tests
...
abstract contract ERC4626Prop is Test {
    ...
    function prop_convertToShares(address caller1, address caller2, uint256 assets) public {
        vm.prank(caller1);
        uint256 res1 = vault_convertToShares(assets); // "MAY revert due to integer overflow caused by an unreasonably
            // large input."
        vm.prank(caller2);
        uint256 res2 = vault_convertToShares(assets); // "MAY revert due to integer overflow caused by an unreasonably
            // large input."
        assertEq(res1, res2);
    }
    ...
    function prop_convertToAssets(address caller1, address caller2, uint256 shares) public {
        vm.prank(caller1);
        uint256 res1 = vault_convertToAssets(shares); // "MAY revert due to integer overflow caused by an unreasonably
            // large input."
        vm.prank(caller2);
        uint256 res2 = vault_convertToAssets(shares); // "MAY revert due to integer overflow caused by an unreasonably
            // large input."
        assertEq(res1, res2);
    }
    ...
    function prop_previewDeposit(address caller, address receiver, address other, uint256 assets) public {
        vm.prank(other);
        uint256 sharesPreview = vault_previewDeposit(assets); // "MAY revert due to other conditions that would also
    ...
    function prop_deposit(address caller, address receiver, uint256 assets) public {
        ...
        assertApproxEqAbs(newCallerAsset, oldCallerAsset - assets, _delta_, "asset"); // NOTE: this may fail if the
            // caller is a contract in which the asset is stored
    ...
    function prop_withdraw(address caller, address receiver, address owner, uint256 assets) public {
        ...
        assertApproxEqAbs(newReceiverAsset, oldReceiverAsset + assets, _delta_, "asset"); // NOTE: this may fail if the
            // receiver is a contract in which the asset is stored
